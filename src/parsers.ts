...
const parser = <T>(callback: (value: T, styles: UnknownStyles, queue: Queue, props: UnknownProps) => void) => (value: unknown): Parser => (styles, queue, props) => {
  if (isFunction(value)) {
...
export const runtime = (key: string, value: unknown) => parser<string>((value, styles) => {
